...
export default function PmChatPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 flex flex-col" style={{ height: "calc(100vh - 3.5rem)" }}>
      <div className="flex items-center justify-between py-3 border-b border-border">
        <div>
          <Link href={`/projects/${projectId}`} className="text-sm text-text-muted hover:text-text">
            ← Back to board
          </Link>
          <h1 className="font-bold text-lg">PM — {project.name}</h1>
        </div>
      </div>
      ...
      <div className="pb-4 pt-2 border-t border-border flex gap-2 items-end">
        <textarea
          value={input}
